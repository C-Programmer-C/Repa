...
const loadRequests = async () => {
  ...
  const result = await requestsApi.getRequests()
  if (result.success) {
    requests.value = result.data
  } else {
    loadingError.value = result.error || 'Ошибка загрузки данных'
  }
...
const currentVolunteer = computed(() => {
  return authStore.user || { id: null, username: 'Волонтер' }
})
...
const respondToRequest = async () => {
  ...
  const result = await requestsApi.respondToRequest(
    currentRequest.value.id, 
    `${currentVolunteer.value.username}: ${currentVolunteer.value.phone}`
  )
...
const completeRequest = async () => {
  ...
  const result = await requestsApi.completeRequest(currentRequest.value.id)
