...
def respond_request(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            app.help_request = hr
            app.full_clean()  # Вызываем валидацию модели
            app.save()
...
def api_requests(request):
    """API endpoint для получения заявок в формате JSON для Unity-приложения"""
    requests = HelpRequest.objects.filter(status='open')
    
    # Формируем данные в нужном формате для Unity
    requests_data = {
        "requests": [
            {
                "id": str(req.id),
                "type": req.get_help_type_display(),
                "description": req.description,
                "latitude": 55.7558 + (req.id / 100),  # Симуляция координат (Москва)
                "longitude": 37.6173 + (req.id / 80)   # Симуляция координат (Москва)
            } for req in requests
        ]
    }
...
# API для отклика на заявку из Unity-приложения
def api_respond(request):
    """API endpoint для отклика на заявку из Unity-приложения"""
    if request.method != 'POST':
    ...
    if not request_id or not volunteer_id:
        return JsonResponse({"error": "Требуются параметры request_id и volunteer_id"}, status=400)
    
    try:
        help_request = HelpRequest.objects.get(id=request_id)
        # Здесь должна быть логика для сохранения отклика
        # в упрощенном варианте просто подтверждаем успех
        return JsonResponse({"success": True, "message": "Отклик принят"})
...
@login_required
@user_passes_test(lambda u: u.is_volunteer())
def unity_map(request):
    """Страница с картой Unity приложения"""
    return render(request, 'core/unity_map.html')

# Представление для Vue.js приложения с использованием CDN
def vue_app_cdn(request):
    """Страница с Vue.js приложением через CDN"""
    return render(request, 'core/vue_cdn.html')